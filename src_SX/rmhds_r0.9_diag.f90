...
!!!      dx = 2._DP * pi / kx(nx)
      ...
      do iz = -nz, nz-1
        ...
        do ix = 0, nnx-1
          do iy = 0, nny-1
             vx2 = vx(ix,iy)*vx(ix,iy)
             vy2 = vy(ix,iy)*vy(ix,iy)
             if( cfl_vx < vx2 ) cfl_vx = vx2
             if( cfl_vy < vy2 ) cfl_vy = vy2
!            cfl_v = max( cfl_v, abs(vx(ix,iy)), abs(vx(ix,iy)) )
          end do
        end do
      end do
        cfl_v = sqrt( max( cfl_vx, cfl_vy ) )
...
! --- local variables
    ...
    real(kind=DP) :: dkp, ka, wk ! revised Mar 12, 2024
    integer       :: ikp
...
! --- k_perp-modes
      ...
      do my = 1, ny
        do mx = 1, nx
          ka  = sqrt( kx(mx)**2 + ky(my)**2 )
          ikp = int( ka / dkp )
          wk  = ka/dkp - dble(ikp)
          if( ikp <= nk-1 ) then
            mode_k(ikp  ) = mode_k(ikp  )                &
                          + ( wr2(mx,my) + wr2(-mx,my) ) &
                          * ( 1._DP - wk )
            mode_k(ikp+1) = mode_k(ikp+1)                &
                          + ( wr2(mx,my) + wr2(-mx,my) ) &
                          * wk
          end if
        end do
...
!    real(kind=DP) :: dk
    real(kind=DP) :: dkp, ka, wk ! revised Nov 29, 2024
    integer       :: ikp
