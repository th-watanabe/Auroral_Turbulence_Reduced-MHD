...
!  real(kind=DP), dimension(0:nny-1) :: wky2
  ...
  complex(kind=DP), dimension(0:nny/2,0:nnx-1) :: wkxy1, wkxy2
  real(kind=DP), dimension(0:nny-1,0:nny-1) :: wxy
...
!                           FFTW_MEASURE)
    ...
    call dfftw_plan_many_dft(plan_backward_x,          &
                           1, nnx, nny/2+1,            &
                           wkxy1, nny/2+1, nny/2+1, 1, & 
                           wkxy2, nny/2+1, nny/2+1, 1, & 
                           FFTW_BACKWARD,              &
                           FFTW_MEASURE)
    call dfftw_plan_many_dft_c2r(plan_backward_y,      &
                           1, nny, nnx,                &
                           wkxy2, nny/2+1, 1, nny/2+1, & 
                           wxy,   nny,     1, nny,     & 
                           FFTW_MEASURE)
    call dfftw_plan_many_dft_r2c(plan_forward_y,       &
                           1, nny, nnx,                &
                           wxy,   nny,     1, nny,     &
                           wkxy2, nny/2+1, 1, nny/2+1, &
                           FFTW_MEASURE)
    call dfftw_plan_many_dft(plan_forward_x,           &
                           1, nnx, nny/2+1,            &
                           wkxy2, nny/2+1, nny/2+1, 1, & 
                           wkxy1, nny/2+1, nny/2+1, 1, & 
                           FFTW_FORWARD,               &
...
!  Execution of FFT
  ...
  complex(kind=DP), dimension(0:nny/2,0:nnx-1) :: w1, w2
  integer :: mx, my
    ...
    do my = 0, ny
      ...
      call dfftw_execute_dft_c2r(plan_backward_y, w2, wwxy)
...
!  Execution of FFT
  ...
  integer :: mx, my
    ...
    ceff_norm = 1._DP / real(nnx * nny, kind=DP)
      ...
      call dfftw_execute_dft_r2c(plan_forward_y, wwxy, w2)
