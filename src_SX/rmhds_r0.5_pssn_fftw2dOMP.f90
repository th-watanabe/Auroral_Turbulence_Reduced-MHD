...
!  real(kind=DP), dimension(0:nny-1,0:nnx-1) :: dfdx, dfdy, dpdx, dpdy, pbxy
  ...
  real(kind=DP), allocatable, dimension(:,:,:) :: dfdx, dfdy, dpdx, dpdy, pbxy

    allocate( dfdx(0:nny-1,0:nnx-1,0:nm-1) )
    allocate( dfdy(0:nny-1,0:nnx-1,0:nm-1) )
    allocate( dpdx(0:nny-1,0:nnx-1,0:nm-1) )
    allocate( dpdy(0:nny-1,0:nnx-1,0:nm-1) )
    allocate( pbxy(0:nny-1,0:nnx-1,0:nm-1) )
...
!$OMP do
    do im = 0, nm-1
      ...
      call pssn_derivative( fk(:,:,im), dfdx(:,:,im), dfdy(:,:,im) )
      call pssn_derivative( pk(:,:,im), dpdx(:,:,im), dpdy(:,:,im) )
...
!  real(kind=DP), dimension(0:nny-1,0:nnx-1) :: fdpdx, fdpdy, dpdx, dpdy, reff
  ...
  complex(kind=DP), allocatable, dimension(:,:,:) :: ikxg, ikyg, ikxp, ikyp
  real(kind=DP), allocatable, dimension(:,:,:) :: fdpdx, fdpdy, dpdx, dpdy, reff

    allocate( ikxg(-nx:nx,0:ny,0:nm-1) )
    allocate( ikyg(-nx:nx,0:ny,0:nm-1) )
    allocate( ikxp(-nx:nx,0:ny,0:nm-1) )
    allocate( ikyp(-nx:nx,0:ny,0:nm-1) )

    allocate( fdpdx(0:nny-1,0:nnx-1,0:nm-1) )
    allocate( fdpdy(0:nny-1,0:nnx-1,0:nm-1) )
    allocate( dpdx(0:nny-1,0:nnx-1,0:nm-1) )
    allocate( dpdy(0:nny-1,0:nnx-1,0:nm-1) )
    allocate( reff(0:nny-1,0:nnx-1,0:nm-1) )
...
!$OMP do
    do im = 0, nm-1
      ...
      do my = 0, ny
        do mx = -nx, nx
          ikxp(mx,my,im) = ui * kx(mx) * pk(mx,my,im)
          ikyp(mx,my,im) = ui * ky(my) * pk(mx,my,im)
        end do
      end do

      call exb_fft_backward( fk(:,:,im),   reff(:,:,im) )
      call exb_fft_backward( ikxp(:,:,im), dpdx(:,:,im) )
      call exb_fft_backward( ikyp(:,:,im), dpdy(:,:,im) )

      do ix = 0, nnx-1
        do iy = 0, nny-1
          fdpdx(iy,ix,im) = reff(iy,ix,im) * dpdx(iy,ix,im)
          fdpdy(iy,ix,im) = reff(iy,ix,im) * dpdy(iy,ix,im)
        end do
      end do

      call exb_fft_forward( fdpdx(:,:,im), ikxg(:,:,im) )
      call exb_fft_forward( fdpdy(:,:,im), ikyg(:,:,im) )

      do my = 0, ny
        do mx = -nx, nx
          dg(mx,my,im) = ui * ( kx(mx) * ikxg(mx,my,im) + ky(my) * ikyg(mx,my,im) )
        end do
      end do
